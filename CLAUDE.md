...
| `scripts/run_antismash.sbatch` | Slurm job: antiSMASH on the benchmark genome. CPU-only like DeepBGC. It takes `--cpus` and hands it to its own module scheduler, but `seff` on the array run shows that scheduler parallelises very little (see `run_antismash_array.sbatch`), so a wide allocation is wasted here too. Paths are explicit at the top of the file | shell, no test |
