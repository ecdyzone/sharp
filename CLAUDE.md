...
| `scripts/merge_predictions.py` | Per-genome baseline outputs → one `predictions.parquet`. Reuses each `convert_<tool>_to_parquet.py` (no reparsing — format assumptions stay in the tested converters); `--tool antismash\|deepbgc\|gecco`; `--contigs` reports genomes that produced no output at all, which would otherwise be scored as "tool found nothing"; **`--manifest genomes.tsv` is required when the pool is keyed by assembly** (a genome database) rather than by contig accession, or that check compares contigs against assembly directory names and flags every genome missing; `--inspect` mode | `test_merge_predictions.py` |
