...
| `scripts/run_antismash_array.sbatch` | Slurm **job array**: antiSMASH over the whole benchmark set, one task per genome (index → line of `analyzed_contigs.txt`). Resumable (skips genomes with existing output), per-genome output dir. Sizing still **UNMEASURED** — submit 1–2 indices, run `seff <jobid>_<index>`, then submit the rest | shell, no test |
